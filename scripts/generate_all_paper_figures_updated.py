#!/usr/bin/env python3
"""
generate_all_paper_figures_updated.py
=====================================
Generates all 8 high-resolution 300 DPI publication figures including:
...
- Fig 3: Pillar 2: Leave-One-Chemistry-Out (LOCO) Zero-Shot Chemical Extrapolation (2'-OMe, 2'-F, PS, LNA, 2'-MOE)
- Fig 4: 577-d Feature Architecture & Dimensionality Breakdown
- Fig 5: Model Performance, Parity Density Scatterplots & ROC Curves
- Fig 6: Positional Tree SHAP Attribution Heatmap (21 positions)
- Fig 7: In Silico Benchmark on FDA Therapeutics (Naked vs ESC vs ESC+)
- Fig 8: Combinatorial Multi-Modification Beam Search Optimization Trajectories on Oncogenes
"""
...
def generate_fig3_loco():
    print("Generating Figure 3: Leave-One-Chemistry-Out (LOCO) Generalization...")
    fig, ax = plt.subplots(figsize=(8, 4.0), dpi=300)
    
    chems = ["2′-O-Methyl\n(2′-OMe)", "2′-Fluoro\n(2′-F)", "Phosphorothioate\n(PS)", "Locked Nucleic Acid\n(LNA)", "2′-MOE\n(Low-N)"]
    records = [6533, 33069, 34347, 24952, 50]
    r_scores = [0.8603, 0.8221, 0.8137, 0.7990, 0.4190]
    colors_bar = ['#0284c7', '#3b82f6', '#10b981', '#8b5cf6', '#f59e0b']
    
    bars = ax.bar(chems, r_scores, color=colors_bar, edgecolor='#334155', width=0.55)
    ax.set_ylabel("Zero-Shot Pearson Correlation (r)", fontsize=9, fontweight='bold')
    ax.set_title("Pillar 2: Chemical Extrapolation (Leave-One-Chemistry-Out LOCO)\nZero-Shot Performance on Unseen Chemical Families using Multi-Slot Representations", 
                 fontsize=9.5, fontweight='bold', pad=10)
    ax.set_ylim(0, 1.0)
    ax.tick_params(axis='x', labelsize=8)
    ax.tick_params(axis='y', labelsize=8)
    ax.axhline(0.80, color='#e11d48', linestyle='--', lw=1.2, label='High-Accuracy Benchmark Threshold (r = 0.80)')
    ax.legend(loc='upper right', fontsize=8)
    
    for i, bar in enumerate(bars):
        yval = bar.get_height()
        n_rec = records[i]
        ax.text(bar.get_x() + bar.get_width()/2.0, yval + 0.02, f"r = {yval:.4f}\n(N={n_rec:,})", 
                ha='center', va='bottom', fontsize=7.5, weight='bold')
        
    plt.tight_layout()
    fig.savefig(FIG_DIR / "Fig3_LOCO_Generalization.png", dpi=300, bbox_inches='tight')
    plt.close()

def generate_fig4_features():
    print("Generating Figure 4: Feature Architecture & Ablation...")
    fig, axes = plt.subplots(1, 2, figsize=(10, 4.0), dpi=300)
    
    feature_names = ['Multi-Slot Chemical Flags\n(Positional One-Hot)', 'RNA Foundation Models\n(RNA-FM & RNA-Ernie)', 'Engineered Biophysics', 'ViennaRNA Thermodynamics']
    dims = [420, 128, 24, 5]
...
def generate_fig5_performance():
    print("Generating Figure 5: Model Performance & Parity Scatterplots...")
    fig, axes = plt.subplots(1, 3, figsize=(12, 4.0), dpi=300)
    np.random.seed(42)
    ...
    plt.tight_layout()
    fig.savefig(FIG_DIR / "Fig5_Model_Performance.png", dpi=300, bbox_inches='tight')
    plt.close()

def generate_fig6_shap():
    print("Generating Figure 6: Positional SHAP Attribution Heatmap...")
    fig, ax = plt.subplots(figsize=(10, 4.5), dpi=300)
    ...
    plt.tight_layout()
    fig.savefig(FIG_DIR / "Fig6_SHAP_Positional_Heatmap.png", dpi=300, bbox_inches='tight')
    plt.close()

def generate_fig7_fda():
    print("Generating Figure 7: FDA Clinical Benchmark...")
    ...
    plt.tight_layout()
    fig.savefig(FIG_DIR / "Fig7_FDA_Clinical_Benchmark.png", dpi=300, bbox_inches='tight')
    plt.close()

def generate_fig8_beam():
    print("Generating Figure 8: Combinatorial Beam Search Trajectories...")
    fig, axes = plt.subplots(1, 2, figsize=(10, 4.0), dpi=300)
    ...
    plt.tight_layout()
    fig.savefig(FIG_DIR / "Fig8_Beam_Search_Optimization.png", dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    generate_fig2()
    generate_fig3_loco()
    generate_fig4_features()
    generate_fig5_performance()
    generate_fig6_shap()
    generate_fig7_fda()
    generate_fig8_beam()
    print("All 8 publication figures successfully generated in D:/Helixx/paper_figures!")
