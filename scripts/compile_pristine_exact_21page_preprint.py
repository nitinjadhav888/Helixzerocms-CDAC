#!/usr/bin/env python3
"""
compile_pristine_exact_21page_preprint.py
=========================================
Compiles the definitive, publication-grade, exactly 21-page bioRxiv preprint for HelixZero:
...
Outputs:
- D:/Helixx/helixzero_full_preprint_20pages.pdf (EXACTLY 21 Pages)
- D:/Helixx/helixzero_full_preprint_20pages.md (Full Unabridged Markdown Manuscript)
"""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_header_footer(self, page_count):
        ...
        if self._pageNumber > 1:
            self.drawString(54, 752, "HelixZero: Hierarchical ML & Biophysical Calibration for Chemically Modified siRNA")
            self.drawRightString(558, 752, "bioRxiv Preprint | Jadhav et al.")
        ...
        self.drawString(54, 32, "bioRxiv Preprint Submitted for Peer Review | Centre for Development of Advanced Computing (C-DAC)")
        self.setStrokeColor(colors.HexColor("#cbd5e1"))
        self.setLineWidth(0.5)
...
def build_pdf():
    print(f"Compiling exact 21-page publication preprint to {OUTPUT_PDF}...")
    ...
    story.append(HRFlowable(width="100%", thickness=0.8, color=colors.HexColor("#0f2942"), spaceAfter=5))
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 2: INTRODUCTION 1.2 & FIGURE 1
    # ═════════════════════════════════════════════════════
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 6: METHODS 2.3 (577-D FEATURES & ONE-HOT) & FIGURE 4
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    story.append(Paragraph("<b>2.3 577-Dimensional Hybrid Feature Architecture</b>", h2_style))
    story.append(Paragraph("HelixZero extracts a 577-dimensional hybrid feature vector spanning three orthogonal representation layers:", body_style))
    story.append(Paragraph("1. <b>Positional One-Hot Chemical Flags (420 Dimensions)</b>: For each nucleotide in the 21-mer sense and antisense strands, 10 orthogonal boolean flags are extracted: <code>is_ribo</code>, <code>is_2OMe</code>, <code>is_2F</code>, <code>is_deoxy</code>, <code>is_LNA</code>, <code>is_GNA</code>, <code>is_PS</code>, <code>is_5P</code>, <code>is_5VP</code>, and <code>is_GalNAc</code>. This yields $10 \\times 21 \\times 2 = 420$ sparse binary flags directly encoding position-specific chemistry.", bullet_style))
    story.append(Paragraph("2. <b>Engineered Biophysical Descriptors (24 Dimensions)</b>: Multi-slot global counts, terminal-to-internal PS ratios, seed region sequence entropy, 5′-anchor MID compatibility, GC skew, and 3′-overhang conformational rigidity.", bullet_style))
    story.append(Paragraph("3. <b>RNA Foundation Model PCA-32 Subspace (128 Dimensions)</b>: Token-level hidden states from RNA-FM (650M) and RNA-Ernie average-pooled and projected onto 32 principal components per strand ($32 \\times 2 \\times 2 = 128\\text{d}$).", bullet_style))
    story.append(Paragraph("4. <b>ViennaRNA Thermodynamic Parameters (5 Dimensions)</b>: Sense MFE, antisense MFE, duplex hybridization $\\Delta G$, ensemble base-pair distance, and global GC fraction.", bullet_style))

    # EMBED FIG 4
    if (FIG_DIR / "Fig4_Feature_Ablation.png").exists():
        story.append(Spacer(1, 4))
        story.append(Image(str(FIG_DIR / "Fig4_Feature_Ablation.png"), width=490, height=190))
        story.append(Paragraph("Fig. 3. 577-Dimensional hybrid feature architecture: (A) Dimension breakdown across feature modules; (B) Empirical ablation on the CMsiRNAdb Homogeneous test set.", caption_style))
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 8: METHODS 2.6 (PENALTIES), TABLE III, 2.7 (BEAM SEARCH), 2.8 (OFF-TARGET)
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 9: RESULTS 3.1 & TABLE IV (BENCHMARKS)
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 10: RESULTS 3.2, 3.3 & FIGURE 3 (MODEL PERFORMANCE)
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # EMBED FIG 3
    if (FIG_DIR / "Fig3_Model_Performance.png").exists():
        story.append(Spacer(1, 4))
        story.append(Image(str(FIG_DIR / "Fig3_Model_Performance.png"), width=490, height=190))
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 11: RESULTS 3.4 (ABLATION), TABLE V & 3.5 (FDA CLINICAL)
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 12: RESULTS 3.5 (TABLE VI), FIGURE 6 & 3.6 (BEAM SEARCH LEAD OPT)
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    # Table VI
    t6_data = [
    ...
    # EMBED FIG 6
    if (FIG_DIR / "Fig6_FDA_Clinical_Benchmark.png").exists():
        story.append(Spacer(1, 4))
        story.append(Image(str(FIG_DIR / "Fig6_FDA_Clinical_Benchmark.png"), width=490, height=180))
        story.append(Paragraph("Fig. 5. In silico clinical validation on FDA-approved therapeutic scaffolds comparing unmodified RNA, ESC, and ESC+ chemistries.", caption_style))
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 13: RESULTS 3.6 (TABLE VII), 3.7 (TABLE VIII) & FIGURE 7
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    # Table VII
    t7_data = [
    ...
    story.append(Paragraph("<b>3.7 Extended In Silico Lead Optimization on Additional Oncogenic Targets</b>", h2_style))
    story.append(Paragraph("We expanded the multi-modification beam search to three additional high-value therapeutic oncogenes: **TP53 (R175H mutant)**, **BRAF (V600E mutant)**, and **EGFR (T790M resistance mutant)** (Table VIII).", body_style))
    ...
    t8 = Table(t8_data, colWidths=[85, 120, 50, 150, 45, 40])
    ...
    story.append(t8)
    story.append(Paragraph("TABLE VIII: Extended Lead Optimization across Additional Oncogenic Targets.", caption_style))

    # EMBED FIG 7
    if (FIG_DIR / "Fig7_Beam_Search_Optimization.png").exists():
        story.append(Spacer(1, 4))
        story.append(Image(str(FIG_DIR / "Fig7_Beam_Search_Optimization.png"), width=490, height=180))
        story.append(Paragraph("Fig. 6. Combinatorial beam search optimization: (A) Potency rescue trajectories across 6 rounds; (B) Chemical diversity preservation in the candidate pool.", caption_style))

    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 14: RESULTS 3.8 (SHAP ATTRIBUTION), TABLE IX & FIGURE 5
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    story.append(Paragraph("<b>3.8 Model Interpretability and Positional SHAP Attribution</b>", h2_style))
    story.append(Paragraph("To understand the biophysical features driving model predictions, we computed Tree SHAP (SHapley Additive exPlanations) values for CatBoost v4 across all 577 feature dimensions across the CMsiRNAdb validation sets. Table IX summarizes the mean absolute SHAP values and biological interpretations across key structural domains.", body_style))
    ...
    t9 = Table(t9_data, colWidths=[85, 55, 95, 45, 45, 165])
    ...
    story.append(t9)
    story.append(Paragraph("TABLE IX: Positional SHAP Attribution across Duplex Structural Domains.", caption_style))

    # EMBED FIG 5
    if (FIG_DIR / "Fig5_SHAP_Positional_Heatmap.png").exists():
        story.append(Spacer(1, 4))
        story.append(Image(str(FIG_DIR / "Fig5_SHAP_Positional_Heatmap.png"), width=490, height=200))
        story.append(Paragraph("Fig. 7. Positional Tree SHAP attribution heatmap across all 21 nucleotide positions of the antisense (guide) strand.", caption_style))

    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 15: DISCUSSION 4.1 & 4.2
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 17: SECTIONS 5, 6, DATA AVAILABILITY, FUNDING
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 19: REFERENCES (29–50) & SUPPLEMENTARY OVERVIEW
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    refs_p2 = [
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#cbd5e1"), spaceAfter=5))
    story.append(Paragraph("SUPPLEMENTARY INFORMATION OVERVIEW", h1_style))
    story.append(Paragraph("This section contains extended data tables providing complete chemical dictionaries, feature index mappings, and patent census breakdowns for the CMsiRNAdb and IEEE Master datasets:", body_style))
    story.append(Paragraph("• <b>Table S1</b>: Controlled Chemical Modification Vocabulary in HelixZero (30 Classes across Sugar, Backbone, and Termini).", bullet_style))
    story.append(Paragraph("• <b>Table S2</b>: Complete Feature Index Mapping for the 577-Dimensional Hybrid Feature Architecture.", bullet_style))
    story.append(Paragraph("• <b>Table S3</b>: Breakdown of Patent Disclosures, Gene Targets, and Cell Lines in CMsiRNAdb & IEEE Master Datasets.", bullet_style))

    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 20: SUPPLEMENTARY TABLES S1 & S2
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    ...
    # ═════════════════════════════════════════════════════════════════════════════
    # PAGE 21: SUPPLEMENTARY TABLE S3 (PATENTS, GENES, CELL LINES)
    # ═════════════════════════════════════════════════════
    story.append(PageBreak())
    story.append(Paragraph("<b>Supplementary Table S3: Breakdown of Patent Disclosures, Gene Targets, and Cell Lines in CMsiRNAdb & IEEE Master Datasets</b>", h2_style))
    ts3_data = [
    ...
    ts3 = Table(ts3_data, colWidths=[65, 60, 60, 150, 165])
    ...
    story.append(ts3)
    story.append(Paragraph("TABLE S3: Breakdown of Patent Disclosures, Gene Targets, and Cell Lines in CMsiRNAdb and the IEEE Master Dataset.", caption_style))

    doc.build(story, canvasmaker=NumberedCanvas)
    print(f"Successfully compiled pristine exact 21-page preprint: {OUTPUT_PDF}")
