#!/usr/bin/env python3
"""
compile_20page_preprint_pdf.py
==============================
Compiles the complete, unabridged 20+ page bioRxiv publication preprint for HelixZero:
...
Outputs: D:/Helixx/helixzero_full_preprint_20pages.pdf
"""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_header_footer(self, page_count):
        ...
        self.drawString(54, 36, "Confidential — Preprint Submitted for Peer Review | Centre for Development of Advanced Computing (C-DAC)")
        self.setStrokeColor(colors.HexColor("#cbd5e1"))
        self.setLineWidth(0.5)
...
def build_pdf():
    print(f"Compiling comprehensive 20+ page preprint to {OUTPUT_PDF}...")
    ...
    story.append(Paragraph("<b>1.1 The Clinical Promise and Biological Architecture of RNAi Therapeutics</b>", h2_style))
    ...
    story.append(Paragraph("<b>1.3 Limitations of Prior Computational Methods</b>", h2_style))
    ...
    story.append(Paragraph("<b>1.4 Summary of Contributions</b>", h2_style))
    story.append(Paragraph("HelixZero overcomes these challenges through six primary technical innovations:", body_style))
    story.append(Paragraph("1. An <b>orthogonal 5-slot nucleotide chemical ontology (<code>NucSlot</code>)</b> that simultaneously models sugar puckering, backbone linkages, base substitutions, 5′-terminal phosphate caps, and covalent delivery ligands without collisions;", bullet_style))
    story.append(Paragraph("2. A <b>577-dimensional hybrid feature architecture</b> integrating 444 multi-slot positional descriptors, 128 foundation model embedding dimensions from RNA-FM (650M) and RNA-Ernie bidirectional transformers, and 5 ViennaRNA thermodynamic parameters;", bullet_style))
    story.append(Paragraph("3. A <b>2-Stage Hierarchical CatBoost Potency Engine (IEEE v5)</b> that explicitly decouples intrinsic concentration-independent thermodynamic potency ($pIC_{50} = -\\log_{10}(IC_{50})$) from dose-aware biological assay response ($0–100\\%$ mRNA knockdown);", bullet_style))
    story.append(Paragraph("4. A <b>3D Structural Graph Attention Network (PyG MEG-mod GNN)</b> capturing conformation-aware spatial dependencies across the ribonucleotide duplex;", bullet_style))
    story.append(Paragraph("5. A <b>deterministic 5-domain biophysical calibration layer</b> adjusting raw ML predictions against nuclease vulnerability, TLR immunogenicity, RISC loading/Ago2 catalytic compatibility (including seed GNA bonuses), thermodynamic asymmetry, and serum exonuclease persistence; and", bullet_style))
    story.append(Paragraph("6. A <b>vectorized combinatorial beam search optimizer</b> screening candidate spaces ($>18,000$ candidates/sec) with chemical diversity preservation and plateau-based early stopping.", bullet_style))
    ...
    story.append(Paragraph("We curated a comprehensive data lake spanning multi-dose patent disclosures, curated databases, and canonical literature benchmarks ($N = 51,838$ total records). Table I summarizes the dataset partitions, sequence counts, chemistry types, and assay protocols.", body_style))
