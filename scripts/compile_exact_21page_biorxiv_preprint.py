#!/usr/bin/env python3
"""
compile_exact_21page_biorxiv_preprint.py
========================================
Compiles the comprehensive, unabridged, 21-page publication preprint for HelixZero:
...
Outputs:
- D:/Helixx/helixzero_full_preprint_20pages.pdf (Target: 21 Pages)
- D:/Helixx/helixzero_full_preprint_20pages.md (Complete Markdown Manuscript)
"""
...
def build_21page_pdf():
    print(f"Compiling complete 21-page publication preprint to {OUTPUT_PDF}...")
    ...
    # ─── PAGE 2: GRAPHICAL OVERVIEW & INTRODUCTION PART 1 ─────────────────────────
    ...
    # ─── PAGE 4: INTRODUCTION PART 3 & CONTRIBUTIONS ──────────────────────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 5: MATERIALS AND METHODS PART 1 (DATASETS & ONTOLOGY) ───────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 6: MATERIALS AND METHODS PART 2 (577-D FEATURES & EMBEDDINGS) ───────
    story.append(PageBreak())
    ...
    story.append(Paragraph("<b>2.4 RNA Foundation Model PCA-32 Embedding Subspace</b>", h2_style))
    story.append(Paragraph("To incorporate semantic language model representations without inducing catastrophic overfitting on small chemical datasets, we extract token-level hidden states from RNA-FM (650M) and RNA-Ernie transformers. The 640-dimensional token tensors are average-pooled across the 21 nucleotides and projected onto a 32-dimensional principal component subspace fitted exclusively on foundation model pretraining distributions. This provides 32 sense + 32 antisense dimensions per model, yielding 128 dense semantic features.", body_style))

    # ─── PAGE 7: MATERIALS AND METHODS PART 3 (HIERARCHICAL ENGINE & GNN) ──────────
    story.append(PageBreak())
    ...
    # ─── PAGE 8: MATERIALS AND METHODS PART 4 (BIOPHYSICAL PENALTIES & BEAM SEARCH)
    story.append(PageBreak())
    ...
    # ─── PAGE 9: RESULTS PART 1 (BENCHMARK TABLE & FIGURES) ───────────────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 10: RESULTS PART 2 (DICHOTOMY & IEEE MULTI-DOSE RESULTS) ────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 11: RESULTS PART 3 (CLINICAL & ONCOGENE CASE STUDIES) ───────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 12: RESULTS PART 4 (EXTENDED ONCOGENIC TRANSCRIPTS) ─────────────────
    story.append(PageBreak())
    story.append(Paragraph("<b>3.7 Extended In Silico Lead Optimization on Additional Oncogenic Targets</b>", h2_style))
    story.append(Paragraph("We expanded the multi-modification beam search to three additional high-value therapeutic oncogenes: **TP53 (R175H mutant)**, **BRAF (V600E mutant)**, and **EGFR (T790M resistance mutant)** (Table VIII). In each case, starting from poorly performing unmodified parent duplexes (predicted knockdown $31.2\text{--}42.0\%$), the beam search rapidly designed position-specific chemical configurations that elevated potency to $79.4\text{--}83.5\%$ while satisfying all safety constraints.", body_style))
    ...
    # ─── PAGE 13: RESULTS PART 5 (SHAP ATTRIBUTION ANALYSIS) ──────────────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 14: DISCUSSION PART 1 (BIOPHYSICS & METRICS) ────────────────────────
    story.append(PageBreak())
    ...
    # ─── PAGE 15: DISCUSSION PART 2 (DOSE DECOUPLING & CLINICAL TRANSLATION) ───────
    story.append(PageBreak())
    ...
    # ─── PAGE 16: LIMITATIONS, CONCLUSION, AND DATA AVAILABILITY ──────────────────
    story.append(PageBreak())
    ...
    # ─── PAGES 17 & 18: REFERENCES (50 FULL CITATIONS) ────────────────────────────
    story.append(PageBreak())
    ...
    for r in refs:
        story.append(Paragraph(r, ParagraphStyle('RefStyle', fontName='Helvetica', fontSize=6.5, leading=8.5, textColor=colors.HexColor("#334155"), spaceAfter=2)))

    # ─── PAGES 19, 20, 21: EXTENDED DATA TABLES S1 THROUGH S4 ────────────────────
    story.append(PageBreak())
    ...
    story.append(PageBreak())
    story.append(Paragraph("<b>Supplementary Table S3: Breakdown of Patent Disclosures and Gene Targets in IEEE Master Dataset</b>", h2_style))
    ts3_data = [
        [Paragraph("Target Gene Symbol", table_header_style), Paragraph("Sample Count (N)", table_header_style), Paragraph("Assay Concentration Range (nM)", table_header_style), Paragraph("Primary Chemistry Format", table_header_style), Paragraph("Clinical / Biological Indication", table_header_style)],
        [Paragraph("TTR", table_cell_center), Paragraph("4,250", table_cell_center), Paragraph("0.01 – 100 nM", table_cell_center), Paragraph("ESC / ESC+ (GalNAc conjugated)", table_cell_style), Paragraph("Hereditary Transthyretin Amyloidosis", table_cell_style)],
        [Paragraph("ALAS1", table_cell_center), Paragraph("3,180", table_cell_center), Paragraph("0.05 – 50 nM", table_cell_center), Paragraph("ESC (2′-OMe/2′-F/PS)", table_cell_style), Paragraph("Acute Hepatic Porphyria", table_cell_style)],
        [Paragraph("HAO1", table_cell_center), Paragraph("2,890", table_cell_center), Paragraph("0.01 – 100 nM", table_cell_center), Paragraph("ESC (GalNAc conjugated)", table_cell_style), Paragraph("Primary Hyperoxaluria Type 1", table_cell_style)],
        [Paragraph("PCSK9", table_cell_center), Paragraph("5,420", table_cell_center), Paragraph("0.01 – 50 nM", table_cell_center), Paragraph("ESC (GalNAc conjugated)", table_cell_style), Paragraph("Hypercholesterolemia / ASCVD", table_cell_style)],
        [Paragraph("LDHA", table_cell_center), Paragraph("2,140", table_cell_center), Paragraph("0.1 – 100 nM", table_cell_center), Paragraph("ESC (2′-OMe/2′-F/PS)", table_cell_style), Paragraph("Primary Hyperoxaluria Type 1 & 2", table_cell_style)],
        [Paragraph("KRAS", table_cell_center), Paragraph("3,650", table_cell_center), Paragraph("0.1 – 50 nM", table_cell_center), Paragraph("ESC+ (5′-VP + GNA@7)", table_cell_style), Paragraph("Oncogenic KRAS (G12D, G12V) Carcinomas", table_cell_style)],
        [Paragraph("MYC", table_cell_center), Paragraph("2,870", table_cell_center), Paragraph("0.05 – 50 nM", table_cell_center), Paragraph("ESC+ (5′-VP + PS)", table_cell_style), Paragraph("Oncogenic Transcription Factor Activation", table_cell_style)],
        [Paragraph("BCL2", table_cell_center), Paragraph("2,410", table_cell_center), Paragraph("0.1 – 100 nM", table_cell_center), Paragraph("ESC (2′-OMe/2′-F)", table_cell_style), Paragraph("B-cell Lymphoma / Apoptosis Resistance", table_cell_style)],
        [Paragraph("Other (12 Genes)", table_cell_center), Paragraph("13,445", table_cell_center), Paragraph("0.01 – 100 nM", table_cell_center), Paragraph("Mixed Multi-Slot Chemistries", table_cell_style), Paragraph("Cardiovascular, Metabolic & Renal Targets", table_cell_style)],
        [Paragraph("Total Master Dataset", table_header_style), Paragraph("40,255", table_header_style), Paragraph("0.01 – 100 nM", table_header_style), Paragraph("Full Controlled Vocabulary", table_header_style), Paragraph("Consolidated In Vitro & In Silico Series", table_header_style)],
    ]
    ts3 = Table(ts3_data, colWidths=[90, 75, 110, 110, 115])
    ...
    story.append(ts3)
    story.append(Paragraph("TABLE S3: Summary Breakdown of Patent Disclosures and Gene Targets in the IEEE Master Dataset.", caption_style))

    doc.build(story, canvasmaker=NumberedCanvas)
    print(f"Successfully generated 21-page preprint PDF: {OUTPUT_PDF}")
