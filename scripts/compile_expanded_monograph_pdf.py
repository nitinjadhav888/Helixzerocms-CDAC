#!/usr/bin/env python3
"""
compile_expanded_monograph_pdf.py
=================================
Compiles the exhaustive, deeply pedagogical master engineering monograph for HelixZero-CMS:
...
Guarantees:
- Zero raw dollar signs or LaTeX formatting (strictly clean, readable Unicode / plain text).
- Lengthy, thorough, pedagogical, step-by-step explanations of each and every implementation detail.
- Full 14 chapters covering biology, chemistry, 8 engineering hurdles, 577-D/190-D feature spaces,
  ML/DL architectures, calibration, biophysical penalties, safety engines, benchmarks, clinical drugs,
  and complete technology stack.
...
class NumberedCanvas(canvas.Canvas):
    """Two-pass canvas that dynamically counts total pages and renders running header/footer."""
...
def build_pdf():
    print(f"Compiling comprehensive master monograph to {OUTPUT_PDF}...")
...
================================================================================================="""
   ...
   5'-p - [G - U - A - A - G - A - C - U - U - G] - [A - G - A - U - G - A - U - C - C] - dT - dT - 3'
    ...
    story.append(Paragraph("1.2 The Eight-Step Catalytic Cycle of Ago2-RISC", h2_style))
    ...
    story.append(Paragraph("1.3 The Five Clinical Failure Modes of Naked (Unmodified) RNA", h2_style))
    ...
    story.append(Paragraph("<b>1. Ultra-Rapid Endonuclease Cleavage (t½ < 5 minutes):</b> Mammalian serum contains high concentrations of secretory endonucleases, primarily belonging to the pancreatic-like Ribonuclease A (RNase A) family. RNase A catalyzes the endonucleolytic cleavage of RNA at pyrimidine junctions (UA, UG, CA). The catalytic mechanism relies on the 2'-hydroxyl (-OH) group of the ribose ring acting as an internal nucleophile. Activated by a catalytic histidine residue (His12 acting as a general base), the 2'-oxygen attacks the adjacent scissile phosphorus atom, forming a trigonal bipyramidal (pentacoordinate) phosphorane transition state. A second histidine (His119 acting as a general acid) donates a proton to the 5'-oxygen leaving group, severing the RNA backbone and yielding a 2',3'-cyclic phosphate intermediate. In human blood, naked siRNA is completely degraded within minutes. DNA is immune to this mechanism because it lacks the 2'-OH nucleophile.", body_style))
    ...
    story.append(Paragraph("1.4 The Medicinal Chemistry Revolution: Synthetic Modifications", h2_style))
...
class NucSlot:
    ...
    story.append(Paragraph("6.3 Detailed Catalogue of the 24 Literature-Engineered Biophysical Features", h2_style))
    story.append(Paragraph("The 24 engineered features in `features_v4.py` represent specific biochemical and structural interactions identified in seminal medicinal chemistry literature:", body_style))

    biofeat_data = [
        [Paragraph("Index", table_cell_header), Paragraph("Feature Name", table_cell_header), Paragraph("Biophysical Meaning & Literature Grounding", table_cell_header)],
        [Paragraph("0", table_cell_bold), Paragraph("as_pos1_bulky_rigid", table_cell), Paragraph("1.0 if antisense position 1 has LNA, MOE, or ENA; captures Elmén 2005 MID pocket steric clash.", table_cell)],
        [Paragraph("1", table_cell_bold), Paragraph("as_pos1_5p_phosphate_mimic", table_cell), Paragraph("1.0 if antisense position 1 has 5'-VP or 5'-P; captures Parmar 2016 MID pocket phosphate anchoring.", table_cell)],
        [Paragraph("2", table_cell_bold), Paragraph("seed_bulky_rigid_frac", table_cell), Paragraph("Fraction of seed nucleotides (pos 2-8) with bulky rigid sugars; captures Bramsen 2009 seed rigidification.", table_cell)],
        [Paragraph("3", table_cell_bold), Paragraph("seed_flexible_exotic_frac", table_cell), Paragraph("Fraction of seed nucleotides with UNA or GNA; captures Schlegel 2022 seed off-target mitigation.", table_cell)],
        [Paragraph("4", table_cell_bold), Paragraph("as_seed_2F_count", table_cell), Paragraph("Count of 2'-F sugars in antisense seed; correlates with high-affinity target duplex nucleation.", table_cell)],
        [Paragraph("5", table_cell_bold), Paragraph("as_cleavage_flexibility", table_cell), Paragraph("Flexibility score across the scissile window (pos 10-12); requires A-form geometry for Ago2 slicing.", table_cell)],
        [Paragraph("6", table_cell_bold), Paragraph("ss_mod_density", table_cell), Paragraph("Overall chemical modification density of sense strand; Allerson 2005 serum stability threshold.", table_cell)],
        [Paragraph("7", table_cell_bold), Paragraph("as_mod_density", table_cell), Paragraph("Overall chemical modification density of antisense strand; balances nuclease defense with Ago2 tolerance.", table_cell)],
        [Paragraph("8", table_cell_bold), Paragraph("terminal_ps_count_ss", table_cell), Paragraph("Number of terminal phosphorothioate linkages on sense strand; Sakamuri 2020 exonuclease cap.", table_cell)],
        [Paragraph("9", table_cell_bold), Paragraph("terminal_ps_count_as", table_cell), Paragraph("Number of terminal phosphorothioate linkages on antisense strand; Sakamuri 2020 exonuclease cap.", table_cell)],
        [Paragraph("10", table_cell_bold), Paragraph("internal_ps_count", table_cell), Paragraph("Total internal PS linkages across duplex; excessive internal PS (>3) induces hepatotoxicity.", table_cell)],
        [Paragraph("11", table_cell_bold), Paragraph("ss_3p_galnac_flag", table_cell), Paragraph("1.0 if sense strand 3'-terminus has GalNAc conjugate; validates ASGPR targeting vector.", table_cell)],
        [Paragraph("12", table_cell_bold), Paragraph("as_5p_galnac_flag", table_cell), Paragraph("1.0 if antisense 5'-terminus has GalNAc; captures Weingärtner 2020 fatal silencing abolition.", table_cell)],
        [Paragraph("13-23", table_cell_bold), Paragraph("thermo_asymmetry_features", table_cell), Paragraph("Khvorova 2003 end stability asymmetry, AU-richness at guide 5'-end, GC clamp at guide 3'-end.", table_cell)]
    ]
    biofeat_t = Table(biofeat_data, colWidths=[35, 140, 345])
    biofeat_t.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,0), colors.HexColor("#0f2942")),
        ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor("#cbd5e1")),
        ('TOPPADDING', (0,0), (-1,-1), 3),
        ('BOTTOMPADDING', (0,0), (-1,-1), 3),
