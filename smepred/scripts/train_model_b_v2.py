"""
train_model_b_v2.py -- Production retrain of Model B on the multi-slot
...
Final model: blend of the v2-schema and legacy-schema CatBoost models
(weights chosen on the large in-distribution validation set; blending is a
standard variance-reduction step, matching the existing production
"ensemble" convention). Only fixed, untuned CatBoost hyperparameters are
used, mirroring the ablation for consistency.
"""
...
from smepred.src.chem_schema import slots_to_legacy_string
from smepred.src.features import extract_phase2
...
CB_PARAMS = dict(iterations=1000, depth=6, learning_rate=0.05, loss_function="RMSE",
                  random_seed=42, verbose=False, early_stopping_rounds=50)
BLEND_W_LEGACY = 0.25  # chosen on in-distribution val (see ablation doc); v2-dominant
...
def featurize_external(df: pd.DataFrame):
    ...
    return X_legacy, X_v2, y_potency
...
def main():
    ...
    print("Training legacy-schema component...")
    m_legacy = CatBoostRegressor(**CB_PARAMS)
    m_legacy.fit(X_tr_leg, y_tr, eval_set=(X_va_leg, y_va))

    print("Training v2-schema component...")
    ...
    m_legacy.save_model(str(MODELS_DIR / "model_b_v2_legacy.cbm"))
    m_v2.save_model(str(MODELS_DIR / "model_b_v2_multislot.cbm"))

    meta = {
        "version": "model_b_v2",
        "phase": "Multi-slot chemistry schema (chem_schema.py) + blend",
        "date": pd.Timestamp.now().isoformat(),
        "architecture": "blend(legacy_schema_catboost, v2_multislot_catboost)",
        "blend_weights": {"legacy": BLEND_W_LEGACY, "v2_multislot": 1 - BLEND_W_LEGACY},
    ...
    print(f"\nSaved model_b_v2_{{legacy,multislot}}.cbm + model_b_v2_meta.json to {MODELS_DIR}")
