"""
Train Model B (Phase 2) — Chemical category encoding + 14 engineered features.
Uses extract_phase2 for training.
_MODIFICATION_MAP: F, M, L, S, D + E(MOE) + 2,3,4,6,8,Q,U,X (HelixZero codes).
"""
...
from src.features import extract_positional_features_batch, extract_phase2, _MODIFICATION_MAP, _MOD_CATEGORIES, _N_POSITIONAL_FLAGS_P2, _CHEM_CATEGORY_NAMES

MODELS_DIR = Path(__file__).parent.parent / "models"
SRCDIR = Path(__file__).parent.parent

print("Model B Phase 2 — Chemical category encoding + engineered features")
print("  _MODIFICATION_MAP:", dict(_MODIFICATION_MAP))
print("  _CHEM_CATEGORIES:", _CHEM_CATEGORY_NAMES)
n_flags = _N_POSITIONAL_FLAGS_P2
n_pos = n_flags * 21 * 2
n_global = (len(_MOD_CATEGORIES) + 9) * 2
n_eng = 14  # engineered features
n_total = n_pos + n_global + 1 + n_eng
print(f"  Positional flags per pos: {n_flags}  Total dim: {n_total}")
print()
...
X_pos = extract_phase2(sense_list, anti_list, bs_list, ba_list, conc_list)
...
if val_metrics:
    print(f"  Model B Phase 2: PCC={val_metrics['pcc']:.4f}  MAE={val_metrics['mae']:.2f}")
...
meta = {
    'version': 7,
    'phase': 'Phase 2 - chemical category encoding',
